...
function validateBody(body) {
  ...
  /** v2 app sends the question ids from when the student loaded the quiz (stable after resume / server restart). */
  let exerciseIdsForValidation = snap.exerciseIds;
  if (Array.isArray(body.publishedExerciseIds) && body.publishedExerciseIds.length > 0) {
    const bank = loadQuizContent();
    const clientIds = [
      ...new Set(
        body.publishedExerciseIds
          .map((x) => Number(x))
          .filter((n) => Number.isInteger(n) && n >= 1 && n <= MAX_EXERCISE_ID),
      ),
    ];
    if (clientIds.length !== body.publishedExerciseIds.length) {
      return { ok: false, error: "exercise_id_range" };
    }
    for (const id of clientIds) {
      if (!bank[String(id)]) return { ok: false, error: "unexpected_exercise_id" };
    }
    exerciseIdsForValidation = clientIds;
  }
  const expected = new Set(exerciseIdsForValidation);
  ...
  for (const eid of exerciseIdsForValidation) {
    if (!seen.has(eid)) return { ok: false, error: "missing_exercise_id" };
  }
  return { ok: true, publishedExerciseIds: exerciseIdsForValidation, quizRevision: snap.revision };
}
