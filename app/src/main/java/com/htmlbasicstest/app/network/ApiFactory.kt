...
object ApiFactory {
    ...
    fun messageForSubmitFailure(e: Throwable): String {
        if (e is HttpException) {
            ...
            if (e.code() == 400) {
                return "Server rejected the submission. Tap Start over, wait for the quiz to load, then try again."
            }
        }
    ...
    private fun messageForServerError(code: String): String = when (code) {
        "quiz_revision" ->
            "This quiz was updated on the server (version changed). Tap Start over, load the quiz again, then retake it."
        "answers_count", "missing_exercise_id", "unexpected_exercise_id", "duplicate_exercise_id" ->
            "Your answers do not match the quiz on the server. Tap Start over and try again."
        "no_active_quiz" ->
            "No test is active on the server right now. Ask your teacher to turn the quiz on."
        "exercise_id_range", "answer_shape" ->
            "Invalid answer data. Tap Start over and try again."
